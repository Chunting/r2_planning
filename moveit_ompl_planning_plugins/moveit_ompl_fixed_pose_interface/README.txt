The rice_moveit_ompl_interface supersedes the existing ompl_interface plugin that is packaged with moveit.  The rice_moveit_ompl_interface interface provides a plugin framework to easily incorporate new OMPL-based planners into the framework without having to recompile the ompl_interface.
...
planner_configs:
  RRTkConfigDefault:
    plugin: rice_moveit_ompl_interface/GeometricPlanningContext  # <-- This line was added
    type: geometric::RRT
    range: 0.0
    goal_bias: 0.05
